...
| `playbook` | `Playbook` | L'objet playbook contenant toutes les connaissances (strategies, pitfalls, templates, definitions, code_snippets) | ✅ Oui |
...
3. **Décision d'ajout** : Pour chaque insight, le Curator détermine :
   ...
   - Dans quelle section l'ajouter (strategies, pitfalls, templates, definitions, code_snippets)
...
| `operations` | `List[Dict[str, Any]]` | Liste des opérations à effectuer. Chaque opération contient :<br>- `type` : "ADD" (pour l'instant, seul type supporté)<br>- `section` : Une des sections du playbook ("strategies", "pitfalls", "templates", "definitions", "code_snippets")<br>- `content` : Le texte du nouveau bullet à ajouter |
| `raw_response` | `Optional[LLMResponse]` | Réponse brute du LLM (pour debugging) |
